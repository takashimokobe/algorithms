...
# no arguments -> List([], 0)
# returns a empty list, with size 0
def empty_list():
    return List([], 0)

# list int value -> list
# puts the value specified at the index in your list. returns a list.
...
def remove(alist, index):
    if (index < 0 or index > length(alist)):
        ...
        newList = List([None] * (length(alist)-1), 0) # create a new List that is one less
...
#less_than sorts your list in descending order: lowest value elements are placed on top and higher value elements are place below
